...
class MercadoPago_Standard_NotificationController extends Mage_Core_Controller_Front_Action{
	...
	public function indexAction(){
		...
		try {
			...
			$core->log("Received notification", 'mercadopago-notification.log', $params);
			
			if(isset($params['checkout']) && $params['checkout'] == "standard"){
				$core->log("Type: Standard", 'mercadopago-notification.log');
				$this->standard($params);
			}else{
				$core->log("Type: Custom", 'mercadopago-notification.log');
				$this->custom($params);
			}
		} catch (Exception $e) {
	...
	*/
	function updateOrder($data){
		...
		$core->log("Update Payment", 'mercadopago-notification.log', $payment_status->toString());
